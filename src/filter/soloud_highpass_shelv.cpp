#include "soloud.h"
#include "soloud_highpass_shelv.h"
#include <math.h>
#include <stdio.h>
namespace SoLoud {
    ...
    void HighPassShelvInstance::calcShelvParams(float f, int order, float gain) {
        ...
        // Inversión de la ganancia porque es Paso Alto
        G = 1/G;
        ...
        case 3:
            ...
            break;
        case 4:
            // Control de K para garantizar la estabilidad del filtro
            if (K < 0.1) {
                K = 0.1;
            }

            // G igual a la raiz de G para hacer las operaciones más legibles
            G = sqrt(G);
        ...
        // Se dividen todos los coeficientes de b porque es paso alto
    ...
    void HighPassShelvInstance::filter(float* aBuffer, unsigned int aSamples, double aTime, unsigned int aChannel, float aSamplerate, int order) {
        
        // Si estamos en el primer canal, se actualizan los parámetros solo si han cambiado.
        if (aChannel == 0) {
            updateParams(aTime);
            if (mParamChanged & ((1 << HighPassShelv::FREQ_C) | (1 << HighPassShelv::ORDER) | (1 << HighPassShelv::GAIN)) || aSamplerate != mSamplerate)
            {
                mSamplerate = aSamplerate;
                calcShelvParams(mParam[HighPassShelv::FREQ_C], mParam[HighPassShelv::ORDER], mParam[HighPassShelv::GAIN]);
            }
            mParamChanged = 0;
        }
    ...
    HighPassShelvInstance::HighPassShelvInstance(HighPassShelv* aParent) {
        mParent = aParent;  
        initParams(4);

        mParam[HighPassShelv::FREQ_C] = aParent->mFreqC;
        mParam[HighPassShelv::ORDER] = aParent->mOrder;
        mParam[HighPassShelv::GAIN] = aParent->mGain;
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 2; j++) {
                x[i][j] = 0.0f;
                y[i][j] = 0.0f;
    ...
    void HighPassShelvInstance::filterChannel(float* aBuffer, unsigned int aSamples, float aSamplerate, double aTime, unsigned int aChannel, unsigned int /*aChannels*/) {
        mSamplerate = aSamplerate;
        filter(aBuffer, aSamples, aTime, aChannel, aSamplerate, mParam[HighPassShelv::ORDER]);
    }

    // Destructor de la instancia
    HighPassShelvInstance::~HighPassShelvInstance() {
    }

    // Constructor de la clase filtro, se setean los parámetros por defecto
    HighPassShelv::HighPassShelv() {
        setParams(100.0f, 1, 0);
    }

    // Función para setear los parámetros del filtro
    result HighPassShelv::setParams(float freqC, int order, float gain) {
    ...
    int HighPassShelv::getParamCount() {
        return 4;
    }
    
    // Función estandar que devuelve el nombre de los parámetros del filtro
    const char* HighPassShelv::getParamName(unsigned int aParamIndex) {
        if (aParamIndex > 3)
            return 0;
        const char* names[4] = {
            "Wet",
    ...
    unsigned int HighPassShelv::getParamType(unsigned int aParamIndex) {
        if(aParamIndex == 2) {
            return INT_PARAM;
        }
        return FLOAT_PARAM;
    ...
    float HighPassShelv::getParamMax(unsigned int aParamIndex)
    {
    ...
    float HighPassShelv::getParamMin(unsigned int aParamIndex)
    {
        if (aParamIndex == 1) {
            return 40.0f;
        }
    ...
    HighPassShelvInstance* HighPassShelv::createInstance() {
        return new HighPassShelvInstance(this);
    }
