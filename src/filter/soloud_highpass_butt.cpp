#include "soloud.h"
#include "soloud_highpass_butt.h"
#include <math.h>
#include <stdio.h>
namespace SoLoud{

    void HighPassButtInstance::calcButtParams(float f, int order){
    ...
    void HighPassButtInstance::filter(float* aBuffer,unsigned int aSamples, double aTime, unsigned int aChannel, float aSamplerate, int order){
        
        // Si estamos en el primer canal, se actualizan los parámetros solo si han cambiado.
        if (aChannel == 0) {
            updateParams(aTime);
            if (mParamChanged & ((1 << HighPassButt::FREQ_C) | (1 << HighPassButt::ORDER) | (1 << HighPassButt::WET)) || aSamplerate != mSamplerate)
            {
                mSamplerate = aSamplerate;
                calcButtParams(mParam[HighPassButt::FREQ_C], mParam[HighPassButt::ORDER]);
            }
            mParamChanged = 0;
        }
    ...
    HighPassButtInstance::HighPassButtInstance(HighPassButt *aParent){
        mParent = aParent;
        initParams(3);

        mParam[HighPassButt:: FREQ_C] = aParent->mFreqC;
        mParam[HighPassButt:: WET] = 1;
        mParam[HighPassButt::ORDER] = aParent->mOrder;
        for(int i = 0; i < 4; i++){
            for(int j = 0; j <2; j++){
                x[i][j] = 0.0f;
                y[i][j] = 0.0f;
    ...
    void HighPassButtInstance::filterChannel(float* aBuffer, unsigned int aSamples, float aSamplerate, double aTime, unsigned int aChannel, unsigned int /*aChannels*/) {
        mSamplerate = aSamplerate;
        filter(aBuffer, aSamples, aTime, aChannel, aSamplerate, mParam[HighPassButt::ORDER]);
    }

    // Destructor de la instancia
    HighPassButtInstance::~HighPassButtInstance(){
    }

    // Constructor de la clase filtro, se setean los parámetros por defectos
    HighPassButt::HighPassButt(){
        setParams(100.0f, 1);   
    }

    // Función para setear los parámetros del filtro
    result HighPassButt::setParams(float freqC, int order){
        if((freqC > 18000.0f || freqC <40.0f || order <= 0 || order >= 4))
    ...
    int HighPassButt::getParamCount(){
        return 3;
    }

    // Función estandar que devuelve el nombre de los parámetros del filtro
    const char* HighPassButt::getParamName(unsigned int aParamIndex){
        if(aParamIndex < 4)
            return 0;
        const char *names[3]={
            "Wet",
    ...
    unsigned int HighPassButt::getParamType(unsigned int aParamIndex){
        if(aParamIndex == 2) {
            return INT_PARAM;
        }
        return FLOAT_PARAM;
    ...
    float HighPassButt::getParamMax(unsigned int aParamIndex)
    {
    ...
    float HighPassButt::getParamMin(unsigned int aParamIndex)
    {
       if(aParamIndex == 1){
            return 40.0f;
        }
    ...
    HighPassButtInstance * HighPassButt::createInstance(){
        return new HighPassButtInstance(this);
    }
